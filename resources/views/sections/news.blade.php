@php
    $news = config('huv.news');
@endphp

<section id="noticias" aria-labelledby="huv-noticias" class="bg-navy text-on-brand">
    <x-container class="py-12 lg:py-14">

        <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
            <h2 id="huv-noticias"
                class="m-0 font-display text-22 font-bold underline decoration-2 underline-offset-8 lg:text-26">
                {{ $news['title'] }}
            </h2>
        ...
        @else
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-10">
                    ...
                    <article class="flex flex-col gap-4">
                        <a href="{{ $featured->url() }}" tabindex="-1" aria-hidden="true"
                           class="block aspect-video overflow-hidden rounded-[3px] bg-white/10">
                            <x-image-slot :src="$featured->imageUrl()"
                                          :alt="''"
                                          hint="Foto de la noticia principal (1000×500)"
                                          :bordered="false" />
                        </a>
                ...
                {{-- Titulares --}}
                <ul class="flex flex-col gap-5 {{ $featured ? '' : 'lg:col-span-2 lg:grid lg:grid-cols-2 lg:gap-8' }}">
                        ...
                        <li>
                            <article class="grid grid-cols-[110px_minmax(0,1fr)] gap-4 sm:grid-cols-[150px_minmax(0,1fr)]">
                                <a href="{{ $item->url() }}" tabindex="-1" aria-hidden="true"
                                   class="block aspect-video overflow-hidden rounded-[3px] bg-white/10">
                                    <x-image-slot :src="$item->imageUrl()"
                                                  :alt="''" hint="Miniatura (1000×500)" :bordered="false" />
                                </a>
