...
@auth
    @php
        ...
        $actions = [
            [
                'label' => $item->is_featured ? 'Ya está destacado' : 'Destacar',
    ...
    <div x-show="$store.huvUi.editMode" x-cloak
        ...
        <button type="button" @click="open = ! open"
            ...
            <span class="sr-only">Acciones de «{{ Str::limit($item->title, 60) }}»</span>
        </button>
            ...
            <ul>
                ...
                <li class="mt-1 border-t border-line pt-1">
                    <form method="POST" action="{{ route('admin.topics.items.destroy', [$topic, $item]) }}"
                          onsubmit="return confirm('¿Eliminar «{{ Str::limit(addslashes($item->title), 60) }}»? La acción no se puede deshacer.')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                class="flex w-full items-center gap-[10px] border-0 bg-transparent px-4 py-[9px]
