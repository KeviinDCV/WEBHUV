...
@auth
    @php
        ...
        $actions = [
            [
                'label' => $item->is_featured ? __('componentes.acciones.ya_destacado') : __('componentes.acciones.destacar'),
                'route' => route('admin.topics.items.feature', [$topic, $item]),
                'disabled' => $item->is_featured,
                'icon' => 'M12 3.5l2.6 5.3 5.9.9-4.3 4.1 1 5.8-5.2-2.7-5.2 2.7 1-5.8L3.5 9.7l5.9-.9z',
            ],
            [
                'label' => $item->is_active ? __('componentes.acciones.inactivar') : __('componentes.acciones.activar'),
                'route' => route('admin.topics.items.active', [$topic, $item]),
                'icon' => 'M12 4v8M7.5 6.3a7.5 7.5 0 1 0 9 0',
            ],
            [
                'label' => $item->is_hidden ? __('componentes.acciones.mostrar') : __('componentes.acciones.ocultar'),
                'route' => route('admin.topics.items.hidden', [$topic, $item]),
                'icon' => $item->is_hidden
    ...
    <div x-show="$store.huvUi.editMode" x-cloak
        ...
        <button type="button" @click="open = ! open"
            ...
            <span class="sr-only">{{ __('componentes.acciones.menu_elemento', ['titulo' => Str::limit($item->title, 60)]) }}</span>
        </button>
        ...
        <div x-show="open" x-cloak
            ...
            <ul>
                ...
                <li class="mt-1 border-t border-line pt-1">
                    <form method="POST" action="{{ route('admin.topics.items.destroy', [$topic, $item]) }}"
                          onsubmit="return confirm('{{ __('componentes.acciones.eliminar_confirmar', ['titulo' => Str::limit(addslashes($item->title), 60)]) }}')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                class="flex w-full items-center gap-[10px] border-0 bg-transparent px-4 py-[9px]
