...
@php
    ...
    */
    $networks = [
        ...
        [
            'name' => __('componentes.compartir.correo'),
...
<div {{ $attributes->class('flex flex-wrap items-center gap-3') }}>
    <span class="text-13-5 font-semibold text-heading">{{ __('componentes.compartir.rotulo') }}</span>
            ...
            <li>
                <a href="{{ $network['url'] }}" target="_blank" rel="noopener noreferrer"
                   class="flex size-8 items-center justify-center rounded-full bg-azure text-on-accent
                    ...
                    <span class="sr-only">{{ __('componentes.compartir.en_red', ['red' => $network['name']]) }}</span>
                </a>
            </li>
        @endforeach
    </ul>
